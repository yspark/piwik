...
class Piwik_Tracker_Visit implements Piwik_Tracker_Visit_Interface
{
  ...
  {
    printDebug("***Piwik_Tracker_Visit::__construct()");
    ...
    printDebug("***ipString:".$ipString.", ip:".Piwik_IP::N2P($ip));
  ...
  {
    printDebug("***Piwik_Tracker_Visit::handle()");
    ...
    {
      printDebug("***Call handleKnownVisit()");
    ...
    //  YSPARK
    printDebug("***SADS starts");
    $this->sendSADSInformation();
  ...
  // YSPARK
  protected function sendSADSInformation()
  {
    printDebug("***sendSADSInformation()");
    ...
    if($socket === false)
    {
      printDebug("***socket_create() failed: ".socket_strerror(socket_last_error()));
    }

    if(socket_connect($socket, $host, $port) === false)
    {
      printDebug("***socket_connect() failed: ".socket_strerror(socket_last_error($socket)));
    }

    //$payload = json_encode($this->visitorInfo);
  ...
  {
    printDebug("***visitorInfo: ".json_encode($this->visitorInfo));

    if(!$this->isVisitorKnown() || !$this->isLastActionInTheSameVisit())
    {
      $payload["newVisit"] =  true;
    }
    else
    {
      $payload["newVisit"] = false;
    }
  ...
  {
    printDebug("***Piwik_Tracker_Visit::handleKnownVisit()");
    printDebug("***Update SQL here");
  ...
  {
    printDebug("***Piwik_Tracker_Visit::handleNewVisit()");
  ...
  {
    printDebug("***Piwik_Tracker_Visit::saveVisitorInformation()");
    printDebug("***SQL:Insert new visitor information here");
